...
const SearchHistoryPage = () => {
	...
	useEffect(() => {
		const getSearchHistory = async () => {
			try {
			    ...
			    toast.error(`Failed to fetch search history: ${error.response?.data?.message || error.message}`);
				setSearchHistory([]);
			}
		};
		getSearchHistory();
    ...
    const handleDelete = async (entry) => {
		try {
			...
			toast.error(`Failed to delete search item: ${error.response?.data?.message || error.message}`);
		}
	};
